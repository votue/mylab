...
class MembersController extends AdminBaseController {
    ...
    public function actionedituser()
    {
        ...
        {
            if( isset($_POST['submit']) && isset( $_POST['Members'] ) )
            {
                $model->attributes = $_POST['Members'];
                $model->scenario = 'update';
                if(!$model->password || $model->password == '') unset($model->password);
                if( $model->save(false) )
                {
                    //var_dump($model->package);exit();
            ...
            */
            ...
            $rows = Board::model()->findAllByAttributes(array('member_id' => $model->id));
            ...
            // Display form
            $this->render('user_form', array( /*'items_selected' => $items_selected, 'items' => $items, */'sort'=>$sort,'rows'=>$rows,'model' => $model, 'label' => Yii::t('adminmembers', 'Editing User') ));
        }
        else
        {
    ...
    public function actiondeleteuser()
    {
        ...
        if( isset($_GET['id']) && ( $model = Members::model()->findByPk($_GET['id']) ) )
        {           
            $boards = Board::model()->findAllByAttributes(array('member_id'=>$model->id));
            if(!empty($boards)){
                foreach ($boards as $board)
                {   
                    $buzzers = Buzzer::model()->findAllByAttributes(array('board_id'=>$board->id));
            ...
            $auth = Authentications::model()->findByAttributes(array('email'=>$model->email));
            if(isset($auth)){
                $auth->delete();
            }
            $model->delete();
