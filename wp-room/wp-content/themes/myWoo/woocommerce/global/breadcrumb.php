...
$my_taxonomy = 'product_woo_custom';
...
if ( ( ! is_home() && ! is_front_page() && ! ( is_post_type_archive() && get_option( 'page_on_front' ) == wc_get_page_id( 'shop' ) ) ) || is_paged() ) {
	...
	if ( is_category() ) {
	...
	} elseif ( is_tax( 'product_woo_custom' ) ) {
		...
		if ( get_post_type() == 'product' ) {
			...
			if ( $terms = wc_get_product_terms( $post->ID, 'product_woo_custom', array( 'orderby' => 'parent', 'order' => 'DESC' ) ) ) {

				$main_term = $terms[0];

				$ancestors = get_ancestors( $main_term->term_id, 'product_woo_custom' );

				$ancestors = array_reverse( $ancestors );

				foreach ( $ancestors as $ancestor ) {
					$ancestor = get_term( $ancestor, 'product_woo_custom' );

					if ( ! is_wp_error( $ancestor ) && $ancestor )
						echo $before . '<a href="' . get_term_link( $ancestor->slug, 'product_woo_custom' ) . '">' . $ancestor->name . '</a>' . $after . $delimiter;
				}

				echo $before . '<a href="' . get_term_link( $main_term->slug, 'product_woo_custom' ) . '">' . $main_term->name . '</a>' . $after . $delimiter;
